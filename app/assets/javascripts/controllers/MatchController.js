...
app.controller('MatchCtrl',['$scope','$http','$timeout','$interval',function($scope,$http,$timeout,$interval){
  ...
  function match_log(str){
    switch (true) {
      case /- Loading maps (\w+)/.test(str):
        ...
        break;
      case /- Teams: (.+) - ([^<>\n]+)(<br>)*/.test(str):
        console.log("Matchinfo Teams found");
        set_teams(str);
        break;
  ...
  function join_team(log){
    ...
    console.log("JOIN TEAM", matches[2], matches[1]);
    if(matches[2] == "CT"){
